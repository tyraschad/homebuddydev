...
import { useSettings, mediumSizes, largeSizes } from "@/lib/settings-store";

export const Route = createFileRoute("/settings/text-size")({
  component: TextSizePage,
  head: () => ({ meta: [{ title: "Text size — Settings" }] }),
});

function TextSizePage() {
  const { theme, textSize, setTextSize } = useSettings();
  const isLarge = textSize === "large";
  const sizes = isLarge ? largeSizes : mediumSizes;
      ...
      <div style={{ padding: "0 16px 16px" }}>
        ...
        >
          ...
          <SegmentToggle
            ...
            rightLabel="L"
            isRight={isLarge}
            onChange={(right) => setTextSize(right ? "large" : "medium")}
            theme={theme}
          />
